function coordinate = calc_coordinate(geometry,equilibrium,coordinate_type,coordinate_location,distance,special_region,region,cell_index)
        ...
        case 'radial'
            ...
            switch distance
                ...
                case 'rho'
                    ...
                    if strcmp(coordinate_location,'bottom_face')
                        temp(radial_length/2+1) = 1;
                        for i = 1:radial_length/2
                            temp(i) = coordinate(i) - (coordinate(radial_length/2+1)-1);
                        end
                        for i = radial_length/2+2:radial_length
                            temp(i) = coordinate(i) - (coordinate(radial_length/2+1)-1);
                        end
                        coordinate = temp;
                    end
                ...
                case 'ds'
                    ...
                    switch coordinate_location
                        ...
                        case 'left_face'
                            ...
                            R_separatrix = geometry.crx(radial_index,radial_length/2+1,1);
                            z_separatrix = geometry.cry(radial_index,radial_length/2+1,1);

                            coordinate = zeros(1,radial_length);

                            coordinate(radial_length/2) = -sqrt((R_separatrix-coordinate_X_left_face(1,radial_length/2))^2+(z_separatrix-coordinate_X_left_face(2,radial_length/2))^2);
                            coordinate(radial_length/2+1) = sqrt((coordinate_X_left_face(1,radial_length/2+1)-R_separatrix)^2+(coordinate_X_left_face(2,radial_length/2+1)-z_separatrix)^2);

                            i = radial_length/2-1;
                            while i>0
                                coordinate(i) = coordinate(i+1)-sqrt((coordinate_X_left_face(1,i+1)-coordinate_X_left_face(1,i))^2+(coordinate_X_left_face(2,i+1)-coordinate_X_left_face(2,i))^2);
                                i = i-1;
                            end

                            i = radial_length/2+2;
                            while i<radial_length+1
                                coordinate(i) = coordinate(i-1)+sqrt((coordinate_X_left_face(1,i)-coordinate_X_left_face(1,i-1))^2+(coordinate_X_left_face(2,i)-coordinate_X_left_face(2,i-1))^2);
                                i = i+1;
                            end
                        ...
                        case 'bottom_face'
                            ...
                            coordinate(radial_length/2+1) = 0;

                            i = radial_length/2;
                            while i>0
                                coordinate(i) = coordinate(i+1)-sqrt((coordinate_X_bottom_face(1,i+1)-coordinate_X_bottom_face(1,i))^2+(coordinate_X_bottom_face(2,i+1)-coordinate_X_bottom_face(2,i))^2);
                                i = i-1;
                            end

                            i = radial_length/2+2;
                            while i<radial_length+1
                                coordinate(i) = coordinate(i-1)+sqrt((coordinate_X_bottom_face(1,i)-coordinate_X_bottom_face(1,i-1))^2+(coordinate_X_bottom_face(2,i)-coordinate_X_bottom_face(2,i-1))^2);
                                i = i+1;
                            end
            ...
            switch distance
                ...
                case 'inner'
                    ...
                    switch coordinate_location
                        ...
                        case 'left_face'
                            coordinate = geometry.dspoledge(:,poloidal_index);
                        case 'bottom_face'
                            coordinate = geometry.dspol(:,poloidal_index);
                    end
                ...
                case 'outer'
                    ...
                    switch coordinate_location
                        ...
                        case 'left_face'
                            coordinate = geometry.dspoledge(:,poloidal_index)-geometry.dspoledge(end,poloidal_index);
                        case 'bottom_face'
                            coordinate = geometry.dspol(:,poloidal_index)-geometry.dspoledge(end,poloidal_index);
                    end
            ...
            switch distance
                ...
                case 'inner'
                    ...
                    switch coordinate_location
                        ...
                        case 'left_face'
                            coordinate = geometry.dsparedge(:,poloidal_index);
                        case 'bottom_face'
                            coordinate = geometry.dspar(:,poloidal_index);
                    end
                ...
                case 'outer'
                    ...
                    switch coordinate_location
                        ...
                        case 'left_face'
                            coordinate = geometry.dsparedge(:,poloidal_index)-geometry.dsparedge(end,poloidal_index);
                        case 'bottom_face'
                            coordinate = geometry.dspar(:,poloidal_index)-geometry.dsparedge(end,poloidal_index);
                    end
                ...
                case 'ds'
                    ...
                    switch coordinate_location
                        ...
                        case 'left_face'
                            ...
                            error('Error: Left-face calculation still to convert in calc_coordinate for unstructured grids')

                        case 'bottom_face'

                            error('Error: Bottom-face calculation still to convert in calc_coordinate for unstructured grids')
