function [rcoI,zcoI,fieldI] = vol_interp(gmtry,field,P0,P1,P2,P3,nint)
%
% Interpolate field in the volume specified by the (R,z) points
% P0, P1, P2, P3 (parallelogram in the (R,z) space)
% 
...
if isplasmagrid(gmtry)
    ...
    elseif gmtry.nncut == 1
        ...
        error('Error: interpolate: wrong number of cuts');
    end
elseif istrianglegrid(gmtry)
    ...
    error('Error: interpolate: wrong gmtry structure');
end

% Construct grid where to perform interpolation
t = linspace(0,1,nint);
L1 = [P0(1)+(P1(1)-P0(1))*t ; P0(2)+(P1(2)-P0(2))*t];
L2 = [P3(1)+(P2(1)-P3(1))*t ; P3(2)+(P2(2)-P3(2))*t];
rcoI = zeros(nint,nint);
zcoI = zeros(nint,nint);
t = linspace(0,1,nint);
for i = 1:nint
    rcoI(i,:) = L1(1,i)+(L2(1,i)-L1(1,i))*t;
    zcoI(i,:) = L1(2,i)+(L2(2,i)-L1(2,i))*t;
end
