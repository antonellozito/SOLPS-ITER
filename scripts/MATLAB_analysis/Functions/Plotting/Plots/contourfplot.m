function [Ch] = contourfplot(grid,field,xinterval,yinterval,scale,fmin,fmax,ncont,balance)
%
...
% Consistency checks
if fmin > fmax
    error('Error: contourfplot: fmin > fmax.');
end
...
if ( ~exist('xinterval','var') || isempty(xinterval) ) && ( ~exist('yinterval','var') || isempty(yinterval) )
    ...
    [Ch(2).C,Ch(2).h] = contourf(rC,zC,f,fcont,'LineStyle','none');
    ...
    [Ch(1).C,Ch(1).h] = contourf(rC,zC,f,fcont,'LineStyle','none');
    ...
    [Ch(3).C,Ch(3).h] = contourf(rC,zC,f,fcont,'LineStyle','none');
...
elseif ( exist('xinterval','var') || ~isempty(xinterval) ) && ( ~exist('yinterval','var') || isempty(yinterval) )
    ...
    rC = r(xinterval,:);
    f  = field(xinterval,:);
    [Ch.C,Ch.h] = contourf(rC,zC,f,fcont,'LineStyle','none');
...
elseif isunstructuredgrid(grid)
    ...
    [Ch.C,Ch.h] = contourf(Xr,Yr,Wr,fcont,'LineStyle','none');
...
elseif istrianglegrid(grid)
    ...
    grid_x = linspace(min(unique_x),max(unique_x),1000);
    grid_y = linspace(min(unique_y),max(unique_y),1000);

    [Xr,Yr,Wr] = griddata(grid.nodes(:,1),grid.nodes(:,2),field_interp,grid_x,grid_y');
    
    [Ch.C,Ch.h] = contourf(Xr,Yr,Wr,fcont,'LineStyle','none');

else

    error('Error: contourfplot: wrong geometric structure');
