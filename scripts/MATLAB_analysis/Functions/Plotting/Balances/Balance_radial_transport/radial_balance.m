function rb = radial_balance(flux,source,res,totname,fluxname,sourcename,gmtry,indbal,ismom,axbal,units,default_region,areaend,areatype,radbaldist,residuals)
%
...
switch areaend
    case 'bottom'
        area_divide_rad = sum(calc_fluxend(area_divide,indbal,'bottom',topix,topiy));
    case 'top'
        area_divide_rad = sum(calc_fluxend(area_divide,indbal,'top',topix,topiy));
    case 'none'
        area_divide_rad = 1;
end
...
[x_rad,x_radedge] = calc_radialcoordinate(gmtry,indbal,default_region,radbaldist);
