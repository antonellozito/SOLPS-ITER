function [axgrid,axbal,axstrat] = figures(transport_mode,quantity,isplot,gmtry,strata_plot)

%
% balance_figures creates axis handles for plotting grid and balance plots
%
%% GRID
...
switch quantity
    ...
    case 'force'
        if length(isplot)>1
            strtmp = sprintf('Force balance: (%s',sprintf('%s,',gmtry.species{isplot}));
            strtmp(end) = ')';
        else
            strtmp = ['Force balance: ',gmtry.species{isplot}];
        end
        title(axgrid,strtmp,'fontweight','normal','fontsize',10);
    case 'momentum'
        if length(isplot)>1
...
switch transport_mode
    case 'parallel'
        ...
        position = get(axbal(1),'Position'); legend(axbal(1),'show','location','southoutside');
        set(axbal(1), 'Position', position);

        % Decomposition of upstream fluxes
        axbal(2) = subplot('position',[width+2*margin 1.15*height width height]); box on; hold on;
        position = get(axbal(2),'Position'); legend(axbal(2),'show','location','southoutside');
        set(axbal(2), 'Position', position);

        % Decomposition of downstream fluxes
        axbal(3) = subplot('position',[2*width+3*margin 1.15*height width height]); box on; hold on;
        position = get(axbal(3),'Position'); legend(axbal(3),'show','location','southoutside');
        set(axbal(3), 'Position', position);

        % Decomposition of poloidally-integrated sources
        axbal(4) = subplot('position',[3*width+4*margin 1.15*height width height]); box on; hold on;
        position = get(axbal(4),'Position'); legend(axbal(4),'show','location','southoutside');
...
switch transport_mode
    case 'parallel'
        ...
        % Total balance
        axbal(5) = subplot('position',[margin 1.15*height width height]); box on; hold on;
        position = get(axbal(5),'Position'); legend(axbal(5),'show','location','southoutside');
        set(axbal(5), 'Position', position);

        % Decomposition of radially-integrated fluxes
        axbal(6) = subplot('position',[width+2*margin 1.15*height width height]); box on; hold on;
        position = get(axbal(6),'Position'); legend(axbal(6),'show','location','southoutside');
        set(axbal(6), 'Position', position);

        % Decomposition of radially-integrated sources
        axbal(7) = subplot('position',[2*width+3*margin 1.15*height width height]); box on; hold on;
        position = get(axbal(7),'Position'); legend(axbal(7),'show','location','southoutside');
