function strata_plots(transport_mode,varargin)

transport_mode = lower(char(transport_mode));

switch transport_mode
    case 'parallel'
        [eirene_atom,eirene_mol,eirene_tion,eirene_rc,titlestr,namestr,gmtry,indbal,nstra,axstrat,axbal,radial_coordinate,poloidal_coordinate,area_divide_rad,area_divide_pol,reverse,ismom] = deal(varargin{:});
%
...
for istra = 1:nstra
    ...
    % Source component due to atom-plasma collisions
    for ie = 1:length(eirene_atom)
        tmp = momfac*sum_radial(transport_mode,eirene_atom{ie}(:,:,istra),indbal,gmtry)/area_divide_pol;
    ...
    % Source component due to molecule-plasma collisions
    for ie = 1:length(eirene_mol)
        tmp = momfac*sum_radial(transport_mode,eirene_mol{ie}(:,:,istra),indbal,gmtry)/area_divide_pol;
        tmp_interp = interp1(x_pol, tmp, x_pol_interp, 'makima', 'extrap');
        if any(tmp)
    ...
    % Source component due to test ion-plasma collisions
    for ie = 1:length(eirene_tion)
        tmp = momfac*sum_radial(transport_mode,eirene_tion{ie}(:,:,istra),indbal,gmtry)/area_divide_pol;
        tmp_interp = interp1(x_pol, tmp, x_pol_interp, 'makima', 'extrap');
        if any(tmp)
    ...
    % Source component due to recombinations
    for ie = 1:length(eirene_rc)
        tmp = momfac*sum_radial(transport_mode,eirene_rc{ie}(:,:,istra),indbal,gmtry)/area_divide_pol;
        tmp_interp = interp1(x_pol, tmp, x_pol_interp, 'makima', 'extrap');
        if any(tmp)
...
if (ymin~=ymax)
    ...
    case 'radial'
        [eirene_atom,eirene_mol,eirene_tion,eirene_rc,titlestr,namestr,gmtry,indbal,nstra,axstrat,axbal,radial_coordinate,poloidal_coordinate,area_divide_rad,area_divide_pol,ismom] = deal(varargin{:});
%
% strata_plots decomposes the EIRENE source components in the different strata for both radial balance plots and poloidal balance plots          
% 
% eirene_atom:     An (nx*ny*nstra) sided matrix for the source component due to atom-plasma collisions
...
DEFAULT_REGION = evalin('base', 'DEFAULT_REGION');
if DEFAULT_REGION(2) == 'm'
    tot_atom = repmat({zeros(size(x_pol))},1,length(eirene_atom));
    tot_mol = repmat({zeros(size(x_pol))},1,length(eirene_mol));
    tot_tion = repmat({zeros(size(x_pol))},1,length(eirene_tion));
    tot_rc = repmat({zeros(size(x_pol))},1,length(eirene_rc)); 
else
    tot_atom = repmat({zeros(size(x_pol_interp))},1,length(eirene_atom));
    tot_mol = repmat({zeros(size(x_pol_interp))},1,length(eirene_mol));
    tot_tion = repmat({zeros(size(x_pol_interp))},1,length(eirene_tion));
...
for istra = 1:nstra
    ...
    for ie = 1:length(eirene_atom)
        ...
        if DEFAULT_REGION(2) == 'm'
            if any(tmp)
                plot(x_pol,tmp,'parent',axstrat(5),'marker','.','linewidth',1,'color',cmap(istra,:),'linestyle',linestyle{ie},'displayname',[sprintf('%s',stratum_name{istra}),namestr{ie}]);
    ...
    for ie = 1:length(eirene_mol)
        ...
        if DEFAULT_REGION(2) == 'm'
            if any(tmp)
                plot(x_pol,tmp,'parent',axstrat(6),'marker','.','linewidth',1,'color',cmap(istra,:),'linestyle',linestyle{ie},'displayname',[sprintf('%s',stratum_name{istra}),namestr{ie}]);
    ...
    for ie = 1:length(eirene_tion)
        ...
        if DEFAULT_REGION(2) == 'm'
            if any(tmp)
                plot(x_pol,tmp,'parent',axstrat(7),'marker','.','linewidth',1,'color',cmap(istra,:),'linestyle',linestyle{ie},'displayname',[sprintf('%s',stratum_name{istra}),namestr{ie}]);
    ...
    for ie = 1:length(eirene_rc)
        ...
        if DEFAULT_REGION(2) == 'm'
            if any(tmp)
                plot(x_pol,tmp,'parent',axstrat(8),'marker','.','linewidth',1,'color',cmap(istra,:),'linestyle',linestyle{ie},'displayname',[sprintf('%s',stratum_name{istra}),namestr{ie}]);
...
for ie = 1:length(eirene_atom)
    if DEFAULT_REGION(2) == 'm'
        plot(x_pol,tot_atom{ie},'marker','.','linewidth',1,'color','k','linestyle',linestyle{ie},'parent',axstrat(5),'displayname',['Sum over strata ',namestr{ie}]);
        plot(x_pol,tot_mol{ie},'marker','.','linewidth',1,'color','k','linestyle',linestyle{ie},'parent',axstrat(6),'displayname',['Sum over strata ',namestr{ie}]);
        plot(x_pol,tot_tion{ie},'marker','.','linewidth',1,'color','k','linestyle',linestyle{ie},'parent',axstrat(7),'displayname',['Sum over strata ',namestr{ie}]);
        plot(x_pol,tot_rc{ie},'marker','.','linewidth',1,'color','k','linestyle',linestyle{ie},'parent',axstrat(8),'displayname',['Sum over strata ',namestr{ie}]);
    else
        plot(x_pol_interp,tot_atom{ie},'linewidth',1,'color','k','linestyle',linestyle{ie},'parent',axstrat(5),'displayname',['Sum over strata ',namestr{ie}]);
        plot(x_pol_interp,tot_mol{ie},'linewidth',1,'color','k','linestyle',linestyle{ie},'parent',axstrat(6),'displayname',['Sum over strata ',namestr{ie}]);
        plot(x_pol_interp,tot_tion{ie},'linewidth',1,'color','k','linestyle',linestyle{ie},'parent',axstrat(7),'displayname',['Sum over strata ',namestr{ie}]);
