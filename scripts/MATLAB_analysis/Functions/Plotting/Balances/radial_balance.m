function rb = radial_balance(transport_mode,varargin)

transport_mode = lower(char(transport_mode));

switch transport_mode
    case 'parallel'
        [flux,source,res,totname,fluxname,sourcename,gmtry,indbal,ismom,axbal,units,default_region,reverse,areaend,areatype,radbaldist,residuals] = deal(varargin{:});
%
...
fluxright = [];
for i=1:size(flux,3)
    fluxleft(:,i) = calc_fluxend(transport_mode,flux(:,:,i),indbal,'left');
    fluxright(:,i) = calc_fluxend(transport_mode,flux(:,:,i),indbal,'right',rightix,rightiy);
end
...
switch quantity
    case 'force'
        ...
        na = sum(tmp(:,:,isplot),3);
        area_divide_rad = calc_fluxend(transport_mode,na,indbal,'left');
    case 'total_pressure'
        area_divide_rad = ones(1,size(fluxleft,1));
    otherwise
        switch areaend
            case 'left'
                area_divide_rad = calc_fluxend(transport_mode,area_divide,indbal,'left');
            case 'right'
                area_divide_rad = calc_fluxend(transport_mode,area_divide,indbal,'right',rightix,rightiy);
            case 'none'
                area_divide_rad = ones(1,size(fluxleft,1));
        end
end
...
% Radial coordinate
x_rad = calc_radialcoordinate(transport_mode,gmtry,indbal,default_region,radbaldist);
...
rb.source_component_names = sourcename;
    ...
    case 'radial'
        [flux,source,res,totname,fluxname,sourcename,gmtry,indbal,ismom,axbal,units,default_region,areaend,areatype,radbaldist,residuals] = deal(varargin{:});
%
% radial_balance makes balance plots with radial resolution (integrating in the poloidal direction) in a given region            
%                                                                                                                                
% flux:        An (nx*ny*nd) sized matrix, where nd is the number of different fluxes into which the total flux is decomposed    
...
% Poloidally-integrated radial fluxes through the cell edges along the radial length of the balance volume
% for each flux component
fluxedge = [];
for i=1:size(flux,3)
    fluxedge(:,i) = sum_poloidaledge(flux(:,:,i),indbal,gmtry);
end
...
switch areaend
    case 'bottom'
        area_divide_rad = sum(calc_fluxend(transport_mode,area_divide,indbal,'bottom',topix,topiy));
    case 'top'
        area_divide_rad = sum(calc_fluxend(transport_mode,area_divide,indbal,'top',topix,topiy));
    case 'none'
        area_divide_rad = 1;
end
...
% Radial coordinates for cell-center quantities and edge-centered quantities
[x_rad,x_radedge] = calc_radialcoordinate(transport_mode,gmtry,indbal,default_region,radbaldist);
x_rad_interp = linspace(min(x_rad),max(x_rad),500);
x_radedge_interp = linspace(min(x_radedge),max(x_radedge),500);

% Total balance with residuals
cmap = gmtry.cmap;
title(axbal(1),'Radial balance','fontweight','normal','fontsize',9);
axis(axbal(1),'tight');
fluxtot = momfac*reversefac*sum(fluxedge,2)./area_divide_rad;
fluxtot_interp = interp1(x_radedge, fluxtot, x_radedge_interp, 'makima', 'extrap');
plot(x_radedge_interp,fluxtot_interp,'parent',axbal(1),'displayname',totname{1},'linewidth',1,'color',cmap(1,:)); cmap=circshift(cmap,-1);
sourcetot = momfac*sum(sourceint,2)./area_divide_rad;
sourcetot_interp = interp1(x_rad, sourcetot, x_rad_interp, 'makima', 'extrap');
plot(x_rad_interp,sourcetot_interp,'parent',axbal(1),'displayname',totname{2},'linewidth',1,'color',cmap(1,:));
coderes = momfac*(resint./area_divide_rad)';
coderes_interp = interp1(x_rad, coderes, x_rad_interp, 'makima', 'extrap');
if residuals
   plot(x_rad_interp,coderes_interp,'-m','parent',axbal(1),'displayname',[totname{3},' (code)'],'linewidth',1);
end
       
% Check the level of agreement between post-calculated and code-calculated residuals

postres = momfac*(sum(sourceint,2)-diff(sum(fluxedge,2)))/area_divide_rad';
postres_interp = interp1(x_rad, postres, x_rad_interp, 'makima', 'extrap');
if residuals
   plot(x_rad_interp,postres_interp,'-g','parent',axbal(1),'displayname',[totname{3},' (post-cal.)'],'linewidth',1);
end
fprintf('Radial balance: the maximum difference between code- and post-calculated residuals is %e%%\n',max(abs((coderes-postres)./coderes)*100));
...
for i=1:size(fluxedge,2)
    if any(fluxedge(:,i))
        fluxcomp(:,i) = momfac*reversefac*fluxedge(:,i)./area_divide_rad;
        fluxcomp_interp(:,i) = interp1(x_radedge, fluxcomp(:,i), x_radedge_interp, 'makima', 'extrap');
        plot(x_radedge_interp,fluxcomp_interp(:,i),'parent',axbal(2),'displayname',fluxname{i},'linewidth',1,'color',cmap(1,:)); cmap=circshift(cmap,-1);
    end
end          
...
for i=1:size(sourceint,2)
    if any(sourceint(:,i))
        sourcecomp(:,i) = momfac*sourceint(:,i)./area_divide_rad;
        sourcecomp_interp(:,i) = interp1(x_rad, sourcecomp(:,i), x_rad_interp, 'makima', 'extrap');
        plot(x_rad_interp,sourcecomp_interp(:,i),'parent',axbal(3),'displayname',sourcename{i},'linewidth',1,'color',cmap(1,:)); cmap=circshift(cmap,-1);
    end
end
...
% Radial coordinates
rb.Radial_coordinate_cell = x_rad;
rb.Radial_coordinate_edge = x_radedge;

% Poloidally-integrated flux (total + components)
%rb.(matlab.lang.makeValidName(sprintf(totname{1}))) = fluxtot;
rb.fluxtot = fluxtot;
for i=1:size(fluxedge,2)
    if any(fluxedge(:,i))
        %rb.(matlab.lang.makeValidName(sprintf('Flux_decomp_%s',fluxname{i}))) = fluxcomp(:,i);
        rb.fluxcomp(:,i) = fluxcomp(:,i);
    end
end

% Poloidally-integrated source (total + components)
%rb.(matlab.lang.makeValidName(sprintf(totname{2}))) = sourcetot;
rb.sourcetot = sourcetot;
for i=1:size(sourceint,2)
    if any(sourceint(:,i))
        %rb.(matlab.lang.makeValidName(sprintf('Source_decomp_%s',sourcename{i}))) = sourcecomp(:,i);
...
% Names
rb.flux_tot_name = totname{1};
rb.flux_component_names = fluxname;
rb.source_tot_name = totname{2};
rb.source_component_names = sourcename;
    otherwise
