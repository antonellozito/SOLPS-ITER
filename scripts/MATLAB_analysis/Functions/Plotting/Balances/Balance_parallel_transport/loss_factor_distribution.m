function loss_factor = loss_factor_distribution(BAL_QUANT,BALFILE,indbal,gmtry,reverse,DEFAULT_REGION,AREAEND,AREATYPE,...
...
for i = 1:size(fluxes,2)
    switch DEFAULT_REGION(2)
        case 'i'
            data(i,:) = fluxes{i}.fluxdowntot./fluxes{end}.fluxdowntot;
        case 'o'
            data(i,:) = fluxes{i}.fluxuptot./fluxes{1}.fluxuptot;
    end
end
switch DEFAULT_REGION(2)
    case 'i'
        data((size(fluxes,2)+1),:) = fluxes{(size(fluxes,2))}.fluxuptot./fluxes{end}.fluxdowntot;
    case 'o'
        data((size(fluxes,2)+1),:) = fluxes{(size(fluxes,2))}.fluxdowntot./fluxes{1}.fluxuptot;
end
data(:,end+1) = data(:,end);
...
if COLORBAR
    ...
    switch BAL_QUANT
    case 'total_pressure'
        name = sprintf('Total pressure loss factor');
    case 'total_energy'
        name = sprintf('%s total energy flux%s loss factor',direction,density);
    end
