function fluxes = distribution_fluxes_toten(balfile,indbal,gmtry,reverse,default_region,areaend,areatype,radbaldist,index_start,index_end)
%
...
fluxes{1} = radial_flux_distribution(...
    cat(3,...
        ...
        fhex_thermj+fhix_inert+fhix_vispar+fhix_anml)/1E6,... % Current-related heat convection
    cat(3,...
        raddive_32+raddive_ecrb+raddive_dia+raddive_cond+raddive_thermj+raddivi_cond+raddivi_32+raddivi_dia+raddivi_ecrb+raddivi_inert+raddivi_vispar+raddivi_anml,... % Radial transport
...
j = 2;
for i = index_start:index_end-1
    indbal(i,:) = zeros(1,size(indbal,2));
    fluxes{j} = radial_flux_distribution(...
        cat(3,...
        fhex_cond,... % Electron heat conduction
        fhix_cond,... % Ion heat conduction
        fhex_32,... % Electron heat convection
