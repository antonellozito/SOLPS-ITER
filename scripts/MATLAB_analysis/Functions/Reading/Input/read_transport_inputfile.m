...
function descriptions = read_transport_inputfile_descriptions(SOLPSTOP, grid_version)
    ...
    % Find TRANSPORT blocks with 'Found in b2.transport.inputfile'
    block_starts = [];
    block_ends = [];
    for iL = 1:length(all_lines)
        ...
        if ~isempty(regexp(ln, '^\*\s+NAMELIST\s+/TRANSPORT/', 'once'))
            if iL < length(all_lines) && ...
               ~isempty(strfind(all_lines{iL+1}, 'Found in b2.transport.inputfile'))
