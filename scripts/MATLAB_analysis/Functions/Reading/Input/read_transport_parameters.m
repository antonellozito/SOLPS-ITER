...
function descriptions = read_transport_descriptions(SOLPSTOP, grid_version)

% Read the TRANSPORT namelist documentation block from b2cdcn.F.
...
%
% There are TWO blocks starting with '* NAMELIST /TRANSPORT/' in b2cdcn.F.
% The correct one is identified by having the line
%   '*    Found in b2.transport.parameters.'
% immediately after the header line. The other block (for
% b2.transport.inputfile) is skipped.
%
...
%   *     continuation text...
    ...
    % Find the TRANSPORT blocks that have 'Found in b2.transport.parameters'
    block_starts = [];
    block_ends = [];
    for iL = 1:length(all_lines)
        ln = all_lines{iL};
        if ~isempty(regexp(ln, '^\*\s+NAMELIST\s+/TRANSPORT/', 'once'))
            % Check if the next line contains 'Found in b2.transport.parameters'
            if iL < length(all_lines) && ...
               ~isempty(strfind(all_lines{iL+1}, 'Found in b2.transport.parameters'))
